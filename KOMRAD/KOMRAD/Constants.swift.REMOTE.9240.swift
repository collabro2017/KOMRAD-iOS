...
struct Constants {
    ...
    struct RegexMacros
    {
        /*static let SET = "\\<\\<(set)\\s(\\$[a-z]+)(\\+)?\\=?\"?([A-Za-z0-9\\s]+)\"?\\>\\>"
        static let WAIT_TYPING = "\\<\\<(wait|typing)\\s?([0-9]+)?(?:s)?\\>\\>"
        static let AUDIO = "\\<\\<(audio)\\s\"([A-Za-z0-9\\s]+)\"\\splay>>"
        static let IF_ELSE_ELSEIF = "\\<\\<(if|else|elseif)\\s(\\$[a-z]+)\\s?([a-z]+)?\\s?([0-9]+)?\\>\\>"
        static let THEN = "then"
        static let PAUSE = "pause"
        static let TIME = "time"
